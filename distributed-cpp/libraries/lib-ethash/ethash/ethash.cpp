...
size_t ethash_get_light_cache_size(int epoch_number) noexcept
{
    return static_cast<size_t>(ethash_calculate_light_cache_num_items(epoch_number)) * sizeof(hash512);
}

uint64_t ethash_get_full_dataset_size(int epoch_number) noexcept
{
    return static_cast<uint64_t>(ethash_calculate_full_dataset_num_items(epoch_number)) * sizeof(hash1024);
}
